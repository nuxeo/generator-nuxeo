...
const delegate = {
  ...
  prompting() {
    ...
    const spinner = ora('Starting Theia IDE...').start();
    ...
    docker.run(this, {
      ...
      volumes: [`${root}:/home/project/${folder}`, `${mvn.locateLocalRepository()}:/home/theia/.m2/repository`],
      mounts: [`type=volume,source=nuxeocli-studio-${folder},target=/home/theia/.theia`],
      image: 'akervern/project-theia:latest',
      ports: [`${this.options.port}:3000`],
      onData: (line) => {
        if (line.match(/Deploy plugins list took/)) {
          spinner.succeed(`Started. Theia IDE available at: http://0.0.0.0:${this.options.port}`);
